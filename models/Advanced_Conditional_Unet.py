...
from process import DynamicLineAndStyleFusion
class Unet(nn.Module):
    ...
    def forward(self, x, time, implicit_conditioning, explicit_conditioning,model_clip,Swin_Encoder,current_step,total_step):
        x = torch.cat((x, explicit_conditioning), dim=1)

        x = self.init_conv(x)


        explicit_conditioning_clip=transforms.Lambda(lambda t: (t + 1) / 2)(explicit_conditioning)
        explicit_conditioning_clip= transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        lineart_feature=CLIP_image(explicit_conditioning_clip,model_clip)#线稿部分的融合
        style_feature=Swin_Encoder(implicit_conditioning)
        conditioning=DynamicLineAndStyleFusion(style_feature,lineart_feature,current_step,total_step)
